...
#include "ble_gamepad_configuration.hpp"
...
static inline uint8_t highByte(uint16_t b) {
    return b >> 8;
}

static inline uint8_t lowByte(uint16_t b) {
    return b & 0xFF;
}
...
uint8_t BleGamepad::specialButtonBitPosition(uint8_t b)
{
    if (b >= POSSIBLESPECIALBUTTONS) {
        ESP_LOGE("GAMEPAD", "Invalid argument, index out of range!");
